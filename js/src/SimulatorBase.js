...
SimulatorBase.prototype.start = function (cols, rows, spawnPoint) {
	...
	this._playfield = window.tetris.Arrays.repeat(rows + 4, cols).map(function (cols) {
...
SimulatorBase.prototype.spawn = function () {
	...
	this._simulator.onPieceSpawn(this._fallingPiece, this._fallingPoint, this._ghostPoint);
...
SimulatorBase.prototype.hardDrop = function () {
	...
	this._simulator.onPieceMove(this._fallingPiece, this._fallingPoint, this._ghostPoint, true);
	this._lock();
	return distance;
};
...
SimulatorBase.prototype._drop = function (shouldLock) {
	...
	if (this._move(this._fallingPiece, this._fallingPoint.addY(-1), false, resolveFunc))
		return 1;
};
...
SimulatorBase.prototype._rotate = (function () {
	...
	function resolve() {
		...
		for (i = 1, n = offset.length; i < n; ++i) {
			...
			if (!this._checkCollision(rotatedPiece, p))
				return p;
		}
	}
	return function (dir) {
		...
		if (!this._active()) {
			this._chargedOperations.push(dir === -1
				? this._rotateLeft
				: this._rotateRight);
			return;
		}
		rotatedPiece = dir === 1
			? this._fallingPiece.rotateRight()
			: this._fallingPiece.rotateLeft();
		for (i = 0, n = this._fallingPiece.offset.length; i < n; ++i)
			offset[i] = this._fallingPiece.offset[i].subtract(rotatedPiece.offset[i]);
		offset.length = n;
		this._kick = false;
...
SimulatorBase.prototype._move = function (piece, point, updateGhost, resolveFunc) {
	var landed;
...
SimulatorBase.prototype._lock = function () {
	...
	if (this._fallingPiece.name === 'T' && this._lastSuccessfulMove === this._rotate) {
		count = 0;
		for (i = 0; i < 4; ++i) {
	...
	this._merge();
	this._simulator.onPieceLock(this._playfield, this._clearLines(), rawTspin, this._kick);
};
